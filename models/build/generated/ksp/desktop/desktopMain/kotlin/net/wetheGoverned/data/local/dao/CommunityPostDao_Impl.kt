  ...
  public override fun observePosts(districtId: String): Flow<List<CommunityPostEntity>> {
    ...
    return createFlow(__db, false, arrayOf("community_posts")) { _connection ->
      ...
      try {
        ...
        val _cursorIndexOfContactInfo: Int = getColumnIndexOrThrow(_stmt, "contactInfo")
        val _cursorIndexOfCreatedAt: Int = getColumnIndexOrThrow(_stmt, "createdAt")
        val _cursorIndexOfExpiresAt: Int = getColumnIndexOrThrow(_stmt, "expiresAt")
        val _cursorIndexOfTagsJson: Int = getColumnIndexOrThrow(_stmt, "tagsJson")
        ...
        while (_stmt.step()) {
          ...
          val _tmp: String
          _tmp = _stmt.getText(_cursorIndexOfKind)
          ...
          if (_stmt.isNull(_cursorIndexOfPrice)) {
            _tmpPrice = null
          } else {
          ...
          if (_stmt.isNull(_cursorIndexOfContactInfo)) {
            _tmpContactInfo = null
          } else {
            _tmpContactInfo = _stmt.getText(_cursorIndexOfContactInfo)
          }
          val _tmpCreatedAt: Long
          _tmpCreatedAt = _stmt.getLong(_cursorIndexOfCreatedAt)
          val _tmpExpiresAt: Long?
          if (_stmt.isNull(_cursorIndexOfExpiresAt)) {
            _tmpExpiresAt = null
          } else {
            _tmpExpiresAt = _stmt.getLong(_cursorIndexOfExpiresAt)
          }
          val _tmpTagsJson: String?
          if (_stmt.isNull(_cursorIndexOfTagsJson)) {
            _tmpTagsJson = null
          } else {
            _tmpTagsJson = _stmt.getText(_cursorIndexOfTagsJson)
          }
    ...
    return createFlow(__db, false, arrayOf("community_posts")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          val _tmpKind: CommunityPostKind
          val _tmp_1: String
          _tmp_1 = _stmt.getText(_cursorIndexOfKind)
