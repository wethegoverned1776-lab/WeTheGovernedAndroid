...
class RoomPollRepository(
    ...
    override fun observeDistrictPolls(districtId: String): Flow<List<CivicPoll>> {
        ...
        return db.pollDao().observePollsHierarchy(districtId, stateId).map { list -> list.map { it.toDomain() } }
    }
    ...
    override fun observePollPosts(pollId: String): Flow<List<PollPost>> =
        db.pollPostDao().getAllPostsForPoll(pollId).map { list -> list.map { it.toDomain() } }

    override fun observeOptionPosts(pollId: String, optionId: String): Flow<List<PollPost>> =
        db.pollPostDao().observeOptionPosts(pollId, optionId).map { list -> list.map { it.toDomain() } }

    override fun observeThreadedPosts(parentPostId: String): Flow<List<PollPost>> =
        db.pollPostDao().observeThreadedPosts(parentPostId).map { list -> list.map { it.toDomain() } }
    ...
    override suspend fun syncPoll(poll: CivicPoll) {
        ...
        // V5 HARDENING: LRU Pruning
        db.pollDao().evictStalePolls(poll.districtId, Clock.System.now().toEpochMilliseconds() - 2592000000L)
    }
...
class RoomResidentRepository(
    ...
    override suspend fun updateProfile(pubKey: String, displayName: String, avatarUrl: String?): Result<ResidentProfile> = runCatching {
        ...
        val p = db.residentProfileDao().getProfile(normalizedPubKey) ?: throw Exception("Not found")
        ...
        publisher?.signPublishImportCivicEvent(
            ...
            tags = listOf(listOf("d", normalizedPubKey), listOf("g", domain.federalHouseId ?: "us")),
            content = CivicJson.encodeToString(ResidentProfileSerializer, domain),
            pubKey = normalizedPubKey
        )
    ...
    override suspend fun createProfile(profile: ResidentProfile) {
        ...
        // V5 HARDENING: Prune profiles not seen in 90 days (Simulated scale)
        db.residentProfileDao().evictStaleProfiles(
            before = Clock.System.now().toEpochMilliseconds() - 7776000000L,
            adminPubKey = NostrConstants.ADMIN_PUBKEY
        )
    }
}
...
class RoomCommunityRepository(
    ...
    override suspend fun createPost(
        ...
        publisher?.signPublishImportCivicEvent(
            ...
            content = CivicJson.encodeToString(CommunityPostSerializer, post),
            pubKey = authorPubKey
        )
...
class RoomAccountRepository(private val db: AppDatabase) : AccountRepository {
    override suspend fun register(account: UserAccount): Result<Unit> = runCatching {
        ...
        db.accountDao().upsertAccount(account.toEntity())
    }
...
class RoomVerificationRequestRepository(private val db: AppDatabase) : VerificationRequestRepository {
    override fun observeRequestsForDistrict(districtId: String): Flow<List<VerificationRequest>> =
        db.verificationRequestDao().observeRequestsForDistrict(districtId).map { list -> list.map { it.toDomain() } }

    override fun observeRequestsForState(stateId: String): Flow<List<VerificationRequest>> =
        db.verificationRequestDao().observeRequestsForState(stateId).map { list -> list.map { it.toDomain() } }

    override suspend fun createRequest(request: VerificationRequest): Result<Unit> = runCatching {
        db.verificationRequestDao().upsertRequest(request.toEntity())
    }
    ...
    override suspend fun getRequest(requestId: String): Result<VerificationRequest> = runCatching {
        db.verificationRequestDao().getRequest(requestId)?.toDomain() ?: throw Exception("Not found")
    }
}
