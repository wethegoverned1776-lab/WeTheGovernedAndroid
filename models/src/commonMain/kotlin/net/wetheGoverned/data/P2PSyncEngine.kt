...
class P2PSyncEngine(
    ...
    fun start() {
        ...
        var currentSubscriptionId: String? = null

        // Subscribe to relevant events
        sessionManager.session
            .onEach { session ->
                val myDistrictId = (session?.districtId ?: "us-fl-06").lowercase()
                val myPubKey = session?.pubKey
                
                // Segmentation for hierarchical sync: us, us-fl, us-fl-06
                val segments = mutableListOf<String>()
                val parts = myDistrictId.split("-")
                var current = ""
                parts.forEach { part ->
                    current = if (current.isEmpty()) part else "$current-$part"
                    segments.add(current)
                }
                if (!segments.contains("us")) segments.add(0, "us")
                val uniqueSegments = segments.distinct()

                println("📡 Sync Engine: Starting subscription for segments $uniqueSegments")
                ...
                val districtFilterG = buildJsonObject {
                    ...
                    put("#g", buildJsonArray {
                        uniqueSegments.forEach { add(JsonPrimitive(it)) }
                    })
                }
                ...
                val newSubId = "wtg_sync_${myDistrictId}_${Clock.System.now().toEpochMilliseconds()}"
                currentSubscriptionId?.let { oldId ->
                    scope.launch { relayManager.unsubscribe(oldId) }
                }
                currentSubscriptionId = newSubId
                
                val filters = listOfNotNull(districtFilterG, districtFilterT, userFilter).toTypedArray()
                println("📡 Sync Engine: Subscribing with ${filters.size} filters to relay mesh (SubID: $newSubId)")
                relayManager.subscribe(newSubId, *filters)
    ...
    private suspend fun pushLocalDataToRelays(session: UserSession) {
        ...
        residentRepository.getProfile(session.pubKey).onSuccess { profile ->
            publisher.signPublishImportCivicEvent(
                ...
                content = CivicJson.encodeToString(ResidentProfileSerializer, profile),
                pubKey = session.pubKey
            )
        }
            ...
            if (poll.authorPubKey == session.pubKey || poll.authorPubKey == NostrConstants.ADMIN_PUBKEY) {
                ...
                if (Secp256k1KeyManager.isValidNostrHex(poll.id)) {
                    publisher.signPublishImportCivicEvent(
                        ...
                        content = CivicJson.encodeToString(CivicPollSerializer, poll),
