...
internal class PollPostDao_Impl(
  __db: RoomDatabase,
) : PollPostDao {
  ...
  init {
    this.__db = __db
    this.__upsertAdapterOfPollPostEntity = EntityUpsertAdapter<PollPostEntity>(object : EntityInsertAdapter<PollPostEntity>() {
      protected override fun createQuery(): String = "INSERT INTO `poll_posts` (`id`,`pollId`,`optionId`,`parentPostId`,`headline`,`authorName`,`content`,`score`,`userVote`,`createdAt`) VALUES (?,?,?,?,?,?,?,?,?,?)"
      ...
      protected override fun createQuery(): String = "UPDATE `poll_posts` SET `id` = ?,`pollId` = ?,`optionId` = ?,`parentPostId` = ?,`headline` = ?,`authorName` = ?,`content` = ?,`score` = ?,`userVote` = ?,`createdAt` = ? WHERE `id` = ?"
  ...
  public override suspend fun upsertPost(post: PollPostEntity): Unit = performSuspending(__db, false, true) { _connection ->
  ...
  public override fun getAllPostsForPoll(pollId: String): Flow<List<PollPostEntity>> {
    ...
    return createFlow(__db, false, arrayOf("poll_posts")) { _connection ->
      ...
      try {
        ...
        val _columnIndexOfHeadline: Int = getColumnIndexOrThrow(_stmt, "headline")
        ...
        val _columnIndexOfUserVote: Int = getColumnIndexOrThrow(_stmt, "userVote")
        ...
        while (_stmt.step()) {
          ...
          if (_stmt.isNull(_columnIndexOfHeadline)) {
            _tmpHeadline = null
          } else {
            _tmpHeadline = _stmt.getText(_columnIndexOfHeadline)
          }
          ...
          _tmpUserVote = _stmt.getLong(_columnIndexOfUserVote).toInt()
          val _tmpCreatedAt: Long
          _tmpCreatedAt = _stmt.getLong(_columnIndexOfCreatedAt)
          _item = PollPostEntity(_tmpId,_tmpPollId,_tmpOptionId,_tmpParentPostId,_tmpHeadline,_tmpAuthorName,_tmpContent,_tmpScore,_tmpUserVote,_tmpCreatedAt)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override fun observeOptionPosts(pollId: String, optionId: String): Flow<List<PollPostEntity>> {
    val _sql: String = "SELECT * FROM poll_posts WHERE pollId = ? AND optionId = ? AND parentPostId IS NULL ORDER BY score DESC"
    return createFlow(__db, false, arrayOf("poll_posts")) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          val _tmpCreatedAt: Long
          _tmpCreatedAt = _stmt.getLong(_columnIndexOfCreatedAt)
          _result = PollPostEntity(_tmpId,_tmpPollId,_tmpOptionId,_tmpParentPostId,_tmpHeadline,_tmpAuthorName,_tmpContent,_tmpScore,_tmpUserVote,_tmpCreatedAt)
        } else {
          _result = null
        }
        _result
