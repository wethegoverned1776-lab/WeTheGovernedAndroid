...
internal class VerificationRequestDao_Impl(
  __db: RoomDatabase,
  ...
  init {
    this.__db = __db
    this.__upsertAdapterOfVerificationRequestEntity = EntityUpsertAdapter<VerificationRequestEntity>(object : EntityInsertAdapter<VerificationRequestEntity>() {
      protected override fun createQuery(): String = "INSERT INTO `verification_requests` (`id`,`requesterPubKey`,`requesterDisplayName`,`email`,`districtId`,`stateId`,`address`,`createdAt`,`status`,`handledByPubKey`,`cachedAt`) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
      ...
      protected override fun createQuery(): String = "UPDATE `verification_requests` SET `id` = ?,`requesterPubKey` = ?,`requesterDisplayName` = ?,`email` = ?,`districtId` = ?,`stateId` = ?,`address` = ?,`createdAt` = ?,`status` = ?,`handledByPubKey` = ?,`cachedAt` = ? WHERE `id` = ?"
  ...
  public override suspend fun upsertRequest(request: VerificationRequestEntity): Unit = performSuspending(__db, false, true) { _connection ->
    __upsertAdapterOfVerificationRequestEntity.upsert(_connection, request)
  }

  public override fun observeRequestsForDistrict(districtId: String): Flow<List<VerificationRequestEntity>> {
    val _sql: String = "SELECT * FROM verification_requests WHERE districtId = ? ORDER BY createdAt DESC"
    return createFlow(__db, false, arrayOf("verification_requests")) { _connection ->
      ...
      try {
        ...
        val _columnIndexOfRequesterDisplayName: Int = getColumnIndexOrThrow(_stmt, "requesterDisplayName")
        ...
        while (_stmt.step()) {
          ...
          _item = VerificationRequestEntity(_tmpId,_tmpRequesterPubKey,_tmpRequesterDisplayName,_tmpEmail,_tmpDistrictId,_tmpStateId,_tmpAddress,_tmpCreatedAt,_tmpStatus,_tmpHandledByPubKey,_tmpCachedAt)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override fun observeRequestsForState(stateId: String): Flow<List<VerificationRequestEntity>> {
    val _sql: String = "SELECT * FROM verification_requests WHERE stateId = ? ORDER BY createdAt DESC"
    return createFlow(__db, false, arrayOf("verification_requests")) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          val _tmpCachedAt: Long
          _tmpCachedAt = _stmt.getLong(_columnIndexOfCachedAt)
          _result = VerificationRequestEntity(_tmpId,_tmpRequesterPubKey,_tmpRequesterDisplayName,_tmpEmail,_tmpDistrictId,_tmpStateId,_tmpAddress,_tmpCreatedAt,_tmpStatus,_tmpHandledByPubKey,_tmpCachedAt)
        } else {
          _result = null
        }
        _result
  ...
  public override suspend fun updateStatus(
    ...
    val _sql: String = "UPDATE verification_requests SET status = ?, handledByPubKey = ? WHERE id = ?"
