...
internal class CommunityPostDao_Impl(
  __db: RoomDatabase,
) : CommunityPostDao {
  ...
  init {
    this.__db = __db
    this.__upsertAdapterOfCommunityPostEntity = EntityUpsertAdapter<CommunityPostEntity>(object : EntityInsertAdapter<CommunityPostEntity>() {
      protected override fun createQuery(): String = "INSERT INTO `community_posts` (`id`,`authorPubKey`,`districtId`,`kind`,`title`,`description`,`price`,`location`,`contactInfo`,`createdAt`,`expiresAt`,`cachedAt`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"
      ...
      protected override fun createQuery(): String = "UPDATE `community_posts` SET `id` = ?,`authorPubKey` = ?,`districtId` = ?,`kind` = ?,`title` = ?,`description` = ?,`price` = ?,`location` = ?,`contactInfo` = ?,`createdAt` = ?,`expiresAt` = ?,`cachedAt` = ? WHERE `id` = ?"
  ...
  public override suspend fun upsertPost(post: CommunityPostEntity): Unit = performSuspending(__db, false, true) { _connection ->
    ...
    return createFlow(__db, false, arrayOf("community_posts")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          _item = CommunityPostEntity(_tmpId,_tmpAuthorPubKey,_tmpDistrictId,_tmpKind,_tmpTitle,_tmpDescription,_tmpPrice,_tmpLocation,_tmpContactInfo,_tmpCreatedAt,_tmpExpiresAt,_tmpCachedAt)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override fun observePostsByKind(districtId: String, kind: CommunityPostKind): Flow<List<CommunityPostEntity>> {
    val _sql: String = "SELECT * FROM community_posts WHERE districtId = ? AND kind = ? ORDER BY createdAt DESC"
    return createFlow(__db, false, arrayOf("community_posts")) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          val _tmpCachedAt: Long
          _tmpCachedAt = _stmt.getLong(_columnIndexOfCachedAt)
          _result = CommunityPostEntity(_tmpId,_tmpAuthorPubKey,_tmpDistrictId,_tmpKind,_tmpTitle,_tmpDescription,_tmpPrice,_tmpLocation,_tmpContactInfo,_tmpCreatedAt,_tmpExpiresAt,_tmpCachedAt)
        } else {
          _result = null
        }
        _result
