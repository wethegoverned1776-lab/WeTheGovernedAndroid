...
internal class ManifestoDao_Impl(
  __db: RoomDatabase,
) : ManifestoDao {
  ...
  init {
    this.__db = __db
    this.__upsertAdapterOfCandidateManifestoEntity = EntityUpsertAdapter<CandidateManifestoEntity>(object : EntityInsertAdapter<CandidateManifestoEntity>() {
      protected override fun createQuery(): String = "INSERT INTO `candidate_manifestos` (`id`,`candidatePubKey`,`districtId`,`title`,`body`,`publishedAt`,`cachedAt`) VALUES (?,?,?,?,?,?,?)"
      ...
      protected override fun bind(statement: SQLiteStatement, entity: CandidateManifestoEntity) {
        ...
        statement.bindLong(6, entity.publishedAt)
        statement.bindLong(7, entity.cachedAt)
      }
    }, object : EntityDeleteOrUpdateAdapter<CandidateManifestoEntity>() {
      protected override fun createQuery(): String = "UPDATE `candidate_manifestos` SET `id` = ?,`candidatePubKey` = ?,`districtId` = ?,`title` = ?,`body` = ?,`publishedAt` = ?,`cachedAt` = ? WHERE `id` = ?"
    ...
    this.__upsertAdapterOfManifestoQuestionEntity = EntityUpsertAdapter<ManifestoQuestionEntity>(object : EntityInsertAdapter<ManifestoQuestionEntity>() {
      protected override fun createQuery(): String = "INSERT INTO `manifesto_questions` (`id`,`manifestoId`,`askerPubKey`,`text`,`askedAt`,`answer`,`answeredAt`) VALUES (?,?,?,?,?,?,?)"
      ...
      protected override fun createQuery(): String = "UPDATE `manifesto_questions` SET `id` = ?,`manifestoId` = ?,`askerPubKey` = ?,`text` = ?,`askedAt` = ?,`answer` = ?,`answeredAt` = ? WHERE `id` = ?"
  ...
  public override suspend fun upsertManifesto(manifesto: CandidateManifestoEntity): Unit = performSuspending(__db, false, true) { _connection ->
    __upsertAdapterOfCandidateManifestoEntity.upsert(_connection, manifesto)
  }

  public override suspend fun upsertQuestion(question: ManifestoQuestionEntity): Unit = performSuspending(__db, false, true) { _connection ->
  ...
  public override fun observeManifestos(districtId: String): Flow<List<CandidateManifestoEntity>> {
    val _sql: String = "SELECT * FROM candidate_manifestos WHERE districtId = ? ORDER BY publishedAt DESC"
    return createFlow(__db, false, arrayOf("candidate_manifestos")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          _item = CandidateManifestoEntity(_tmpId,_tmpCandidatePubKey,_tmpDistrictId,_tmpTitle,_tmpBody,_tmpPublishedAt,_tmpCachedAt)
          _result.add(_item)
        }
        _result
      } finally {
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          val _tmpCachedAt: Long
          _tmpCachedAt = _stmt.getLong(_columnIndexOfCachedAt)
          _result = CandidateManifestoEntity(_tmpId,_tmpCandidatePubKey,_tmpDistrictId,_tmpTitle,_tmpBody,_tmpPublishedAt,_tmpCachedAt)
        } else {
          _result = null
        }
        _result
  ...
  public override suspend fun getQuestions(manifestoId: String): List<ManifestoQuestionEntity> {
    val _sql: String = "SELECT * FROM manifesto_questions WHERE manifestoId = ? ORDER BY askedAt ASC"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
        ...
        while (_stmt.step()) {
          ...
          _item = ManifestoQuestionEntity(_tmpId,_tmpManifestoId,_tmpAskerPubKey,_tmpText,_tmpAskedAt,_tmpAnswer,_tmpAnsweredAt)
          _result.add(_item)
        }
        _result
      } finally {
