...
internal class VoteDao_Impl(
  ...
  init {
    this.__db = __db
    this.__upsertAdapterOfCivicVoteEntity = EntityUpsertAdapter<CivicVoteEntity>(object : EntityInsertAdapter<CivicVoteEntity>() {
      protected override fun createQuery(): String = "INSERT INTO `poll_votes` (`id`,`pollId`,`voterPubKey`,`voterName`,`optionId`,`timestamp`,`nonce`,`signature`,`isFlagged`,`flagReason`,`disputeComment`,`disputeExpiresAt`,`status`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
      ...
      protected override fun createQuery(): String = "UPDATE `poll_votes` SET `id` = ?,`pollId` = ?,`voterPubKey` = ?,`voterName` = ?,`optionId` = ?,`timestamp` = ?,`nonce` = ?,`signature` = ?,`isFlagged` = ?,`flagReason` = ?,`disputeComment` = ?,`disputeExpiresAt` = ?,`status` = ? WHERE `id` = ?"
  ...
  public override suspend fun upsertVote(vote: CivicVoteEntity): Unit = performSuspending(__db, false, true) { _connection ->
  ...
  public override fun observeAllVotes(): Flow<List<CivicVoteEntity>> {
    ...
    return createFlow(__db, false, arrayOf("poll_votes")) { _connection ->
      ...
      try {
        ...
        val _columnIndexOfPollId: Int = getColumnIndexOrThrow(_stmt, "pollId")
        val _columnIndexOfVoterPubKey: Int = getColumnIndexOrThrow(_stmt, "voterPubKey")
        val _columnIndexOfVoterName: Int = getColumnIndexOrThrow(_stmt, "voterName")
        val _columnIndexOfOptionId: Int = getColumnIndexOrThrow(_stmt, "optionId")
        ...
        val _columnIndexOfIsFlagged: Int = getColumnIndexOrThrow(_stmt, "isFlagged")
        val _columnIndexOfFlagReason: Int = getColumnIndexOrThrow(_stmt, "flagReason")
        val _columnIndexOfDisputeComment: Int = getColumnIndexOrThrow(_stmt, "disputeComment")
        val _columnIndexOfDisputeExpiresAt: Int = getColumnIndexOrThrow(_stmt, "disputeExpiresAt")
        ...
        while (_stmt.step()) {
          ...
          _tmpPollId = _stmt.getText(_columnIndexOfPollId)
          val _tmpVoterPubKey: String
          _tmpVoterPubKey = _stmt.getText(_columnIndexOfVoterPubKey)
          val _tmpVoterName: String
          _tmpVoterName = _stmt.getText(_columnIndexOfVoterName)
          val _tmpOptionId: String
          _tmpOptionId = _stmt.getText(_columnIndexOfOptionId)
          ...
          val _tmp: Int
          _tmp = _stmt.getLong(_columnIndexOfIsFlagged).toInt()
          _tmpIsFlagged = _tmp != 0
          val _tmpFlagReason: String?
          if (_stmt.isNull(_columnIndexOfFlagReason)) {
            _tmpFlagReason = null
          } else {
            _tmpFlagReason = _stmt.getText(_columnIndexOfFlagReason)
          }
          val _tmpDisputeComment: String?
          if (_stmt.isNull(_columnIndexOfDisputeComment)) {
            _tmpDisputeComment = null
          } else {
            _tmpDisputeComment = _stmt.getText(_columnIndexOfDisputeComment)
          }
          val _tmpDisputeExpiresAt: Long?
          if (_stmt.isNull(_columnIndexOfDisputeExpiresAt)) {
            _tmpDisputeExpiresAt = null
          } else {
            _tmpDisputeExpiresAt = _stmt.getLong(_columnIndexOfDisputeExpiresAt)
          }
          ...
          _item = CivicVoteEntity(_tmpId,_tmpPollId,_tmpVoterPubKey,_tmpVoterName,_tmpOptionId,_tmpTimestamp,_tmpNonce,_tmpSignature,_tmpIsFlagged,_tmpFlagReason,_tmpDisputeComment,_tmpDisputeExpiresAt,_tmpStatus)
          _result.add(_item)
        }
        _result
      } finally {
    ...
    return performSuspending(__db, true, false) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          _tmp_1 = _stmt.getText(_columnIndexOfStatus)
          _tmpStatus = __civicConverters.toConflictStatus(_tmp_1)
          _result = CivicVoteEntity(_tmpId,_tmpPollId,_tmpVoterPubKey,_tmpVoterName,_tmpOptionId,_tmpTimestamp,_tmpNonce,_tmpSignature,_tmpIsFlagged,_tmpFlagReason,_tmpDisputeComment,_tmpDisputeExpiresAt,_tmpStatus)
        } else {
          _result = null
        }
        _result
  ...
  public override suspend fun flagVote(voteId: String, expiresAt: Long) {
    val _sql: String = "UPDATE poll_votes SET isFlagged = 1, status = 'FLAGGED', disputeExpiresAt = ? WHERE id = ?"
