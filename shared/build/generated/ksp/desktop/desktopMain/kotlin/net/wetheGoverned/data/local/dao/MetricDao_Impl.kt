...
internal class MetricDao_Impl(
  ...
  init {
    this.__db = __db
    this.__insertAdapterOfDistrictMetricEntity = object : EntityInsertAdapter<DistrictMetricEntity>() {
      protected override fun createQuery(): String = "INSERT OR REPLACE INTO `district_metrics` (`id`,`districtId`,`category`,`name`,`officialValue`,`residentValue`,`unit`,`source`,`reportedAt`,`reporterPubKey`,`cachedAt`) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
    ...
    this.__upsertAdapterOfDistrictMetricEntity = EntityUpsertAdapter<DistrictMetricEntity>(object : EntityInsertAdapter<DistrictMetricEntity>() {
      protected override fun createQuery(): String = "INSERT INTO `district_metrics` (`id`,`districtId`,`category`,`name`,`officialValue`,`residentValue`,`unit`,`source`,`reportedAt`,`reporterPubKey`,`cachedAt`) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
      ...
      protected override fun createQuery(): String = "UPDATE `district_metrics` SET `id` = ?,`districtId` = ?,`category` = ?,`name` = ?,`officialValue` = ?,`residentValue` = ?,`unit` = ?,`source` = ?,`reportedAt` = ?,`reporterPubKey` = ?,`cachedAt` = ? WHERE `id` = ?"
  ...
  public override suspend fun upsertMetrics(metrics: List<DistrictMetricEntity>): Unit = performSuspending(__db, false, true) { _connection ->
    __insertAdapterOfDistrictMetricEntity.insert(_connection, metrics)
  }

  public override suspend fun upsertMetric(metric: DistrictMetricEntity): Unit = performSuspending(__db, false, true) { _connection ->
  ...
  public override fun observeMetrics(districtId: String): Flow<List<DistrictMetricEntity>> {
    val _sql: String = "SELECT * FROM district_metrics WHERE districtId = ? ORDER BY reportedAt DESC"
    return createFlow(__db, false, arrayOf("district_metrics")) { _connection ->
      ...
      try {
        ...
        while (_stmt.step()) {
          ...
          _item = DistrictMetricEntity(_tmpId,_tmpDistrictId,_tmpCategory,_tmpName,_tmpOfficialValue,_tmpResidentValue,_tmpUnit,_tmpSource,_tmpReportedAt,_tmpReporterPubKey,_tmpCachedAt)
          _result.add(_item)
        }
        _result
      } finally {
